...
- Gate low-evidence high-score findings now use `meta.branch_threshold` when present (default `0.50`) and flag evidence below `0.70`.
- Gate finding sections intentionally render open by default so reviewers immediately see gate failures.
- Gate input intentionally gives `--input FILE` precedence over stdin/noninteractive stdin.
